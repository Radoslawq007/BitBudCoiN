...
const app = express();
// Serwer słucha wyłącznie na 127.0.0.1 (patrz app.listen niżej) - cały ruch
// przychodzi przez Caddy na loopbacku. Bez tego Express mógł widzieć adres
// Caddy zamiast prawdziwego adresu klienta, więc limiter liczyłby CAŁY ruch
// (Twój + wszystkich górników) jako jedną osobę - wyczerpując się natychmiast.
...
app.post("/swap/offers", strictLimiter, (req, res) => {
    const { chain, bbcAmount, expectedAmount, timeoutHours, note } = req.body || {};
    try {
        const offer = swapOffers.createOffer({ chain, bbcAmount, expectedAmount, timeoutHours, note });
...
app.post("/swap/offers/:id/accept", strictLimiter, (req, res) => {
    const { secret, sellerPubKeyHash, sellerBbcAddress } = req.body || {};
    if (secret !== swapOffers.ADMIN_SECRET) {
        return res.status(403).json({ error: "Zły sekret" });
    }
    try {
        const offer = swapOffers.acceptOffer(req.params.id, { sellerPubKeyHash, sellerBbcAddress });
        res.json({ ok: true, offer });
    } catch (err) {
        res.status(400).json({ error: err.message });
    }
...
app.post("/swap/offers/:id/reject", strictLimiter, (req, res) => {
    ...
    try {
        const offer = swapOffers.rejectOffer(req.params.id);
        res.json({ ok: true, offer });
    } catch (err) {
        res.status(400).json({ error: err.message });
    }
...
app.get("/stats/active-addresses", (req, res) => {
    res.json(blockchain.getActiveAddresses());
});

app.get("/stats/new-addresses", (req, res) => {
    const days = Math.min(Number(req.query.days) || 7, 90);
    res.json(blockchain.getNewAddressesPerDay(days));
});

app.post("/family/message", strictLimiter, (req, res) => {
    const result = familyChat.postMessage(req.body);
    if (!result.accepted) return res.status(400).json(result);
    res.json(result);
});

app.get("/family/messages", (req, res) => {
    const limit = Math.min(Number(req.query.limit) || 50, 200);
    res.json(familyChat.getMessages(limit));
});

app.get("/family/live-errors", (req, res) => {
    res.json(familyChat.getLiveErrors());
});

app.get("/family/pending", (req, res) => {
    const secret = req.query.secret;
    if (secret !== bridgeTags.ADMIN_SECRET) return res.status(403).json({ error: "Zły sekret" });
    res.json(familyChat.getPending());
});

app.post("/family/pending/:id/review", strictLimiter, (req, res) => {
    const { secret, decision } = req.body || {};
    if (secret !== bridgeTags.ADMIN_SECRET) return res.status(403).json({ error: "Zły sekret" });
    const result = familyChat.reviewPending(Number(req.params.id), decision);
    if (!result.accepted) return res.status(400).json(result);
    res.json(result);
});
