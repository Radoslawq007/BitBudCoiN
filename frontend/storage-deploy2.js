...
class Storage {
    ...
    saveMempoolTx(tx) {
        // ?? null dla KAŻDEGO pola - transakcje HTLC (create/claim/refund) nie
        // mają tych samych pól co zwykły "transfer" (np. HTLC_CREATE nie ma
        // "to" wcale, HTLC_CLAIM/REFUND nie mają "fee") - node:sqlite rzuca
        // błąd na "undefined", ale akceptuje "null" (kolumny mempool nie mają
        // NOT NULL, więc to bezpieczne).
        this.db.prepare("INSERT OR REPLACE INTO mempool (signature, from_address, to_address, amount, fee, timestamp, publicKey) VALUES (?, ?, ?, ?, ?, ?, ?)")
            .run(tx.signature ?? null, tx.from ?? null, tx.to ?? null, tx.amount ?? null, tx.fee ?? null, tx.timestamp ?? null, tx.publicKey ?? null);
    }
