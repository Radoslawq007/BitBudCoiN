...
class Blockchain {
    ...
    // Adresy aktywne w ostatnim oknie czasowym (domyślnie 24h). Idzie od
    // KOŃCA łańcucha wstecz i zatrzymuje się, jak tylko trafi na blok
    // starszy niż okno - nie skanuje całej historii niepotrzebnie, ważne
    // przy dziesiątkach tysięcy bloków.
    ...
    // Ile NOWYCH (nigdy wcześniej niewidzianych) adresów pojawiło się w
    // każdym z ostatnich `days` dni. Wymaga przejścia całej historii OD
    // POCZĄTKU, żeby wiedzieć czy dany adres naprawdę widziany jest
    // pierwszy raz (nie tylko pierwszy raz w oknie) - liczone raz, dla
    // każdego dnia osobno, nie ponownie od zera.
